...
public class BookmarkRest {
	...
	public Response get( @PathParam("id") Long id ) {
		...
		if( bookmark != null ) {
			log.info("bookmark: " + bookmark.getName() + bookmark.getDescription());
			return Response
					.status( Status.OK )
					.entity( bookmark )
	...
	public Response create( Bookmark value ) {
		...
		log.debug( "create" );
		log.info("新增加的bookmark：" + value.getDescription() + "=======" +  value.getName());
	...
	public Response update( Bookmark value ) {
		...
		log.debug( "update" );
		log.info("需要更新的值： " + value.getDescription() + "==============" + value.getName());
		Bookmark bookmark = dao.save( value );
